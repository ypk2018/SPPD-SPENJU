...
import { SppdData, DocumentType, PaperSize } from './types';
...
export default function App() {
  ...
  // Paper size setting (F4 / Folio 215x330mm or Legal 216x356mm)
      ...
      <header className="bg-gradient-to-r from-blue-900 via-blue-800 to-indigo-900 text-white shadow-md no-print sticky top-0 z-20">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 py-3 flex flex-wrap justify-between items-center gap-3">
          ...
          <div className="flex items-center space-x-2.5">
            {/* Paper Size Switcher */}
            <div className="flex items-center bg-blue-950/70 p-1 rounded-xl border border-white/20 text-xs">
              <span className="text-blue-200 text-[11px] px-2 font-medium hidden md:inline">Kertas:</span>
              <button
                type="button"
                id="btn-paper-f4"
                onClick={() => {
                  setPaperSize('F4');
                  showToast('Ukuran Kertas diatur ke F4 / Folio (215 × 330 mm)');
                }}
                ...
                title="F4 / Folio (215 × 330 mm) - Standar Resmi Kedinasan"
              >
                F4 (Folio)
              </button>
              <button
                ...
                onClick={() => {
                  setPaperSize('LEGAL');
                  showToast('Ukuran Kertas diatur ke Legal (216 × 356 mm)');
                }}
                className={`px-2.5 py-1 rounded-lg font-bold transition-all text-xs ${
                  paperSize === 'LEGAL'
                    ? 'bg-blue-600 text-white shadow-xs'
            ...
            <button 
              ...
              title={`Cetak atau simpan ke PDF ukuran ${paperSize}`}
            >
      ...
      <div className="flex-1 flex overflow-hidden flex-col lg:flex-row">
        ...
        <div className="flex-1 flex flex-col bg-slate-200/70 overflow-hidden">
          ...
          <div className="bg-white border-b border-slate-200 px-4 sm:px-6 flex items-center justify-between no-print overflow-x-auto shadow-xs">
            ...
            {/* Quick Helper Info */}
            <div className="hidden xl:flex items-center text-slate-600 text-xs pl-4 border-l border-slate-200 space-x-2">
              <span className="inline-flex items-center px-2 py-0.5 rounded text-[11px] font-semibold bg-emerald-100 text-emerald-800 border border-emerald-300">
                ✓ Format Kertas {paperSize === 'F4' ? 'F4 / Folio (215 × 330 mm)' : 'Legal (216 × 356 mm)'}
              </span>
              <span className="text-slate-500 text-[11px]">Margin atas/bawah 12mm, samping 18mm (Pas 1 Lembar)</span>
            </div>
          </div>
          ...
          <style>
            {`@media print {
              @page {
                size: ${paperSize === 'LEGAL' ? '216mm 356mm' : '215mm 330mm'};
          ...
          <div className="flex-1 overflow-y-auto p-4 sm:p-6 lg:p-8" id="printable-area">
            {activeTab === 'ALL' && <PrintAllDocuments data={currentRecord} paperSize={paperSize} />}
            {activeTab === 'SP' && <SuratTugas data={currentRecord} paperSize={paperSize} />}
            {activeTab === 'SPPD' && <SuratPerjalananDinas data={currentRecord} paperSize={paperSize} />}
            {activeTab === 'SPPD_BELAKANG' && <SuratPerjalananDinasBelakang data={currentRecord} paperSize={paperSize} />}
            {activeTab === 'RINCIAN' && <RincianBiaya data={currentRecord} paperSize={paperSize} />}
          </div>
